...
export default function Signup({ isLight }) {
  ...
  return (
    <div className={isLight ? 'flex min-h-screen items-center justify-center bg-gray-100'
    :  'flex min-h-screen items-center justify-center bg-[#1f2937]' 
    }>
      <div className={isLight ? "w-full max-w-md rounded-2xl bg-white p-8 shadow-lg"
        :
        "w-full max-w-md rounded-2x bg-gray-600 p-8 shadow-lg text-white"
        }>
        {/* Header */}
        <h1 className={isLight ? "text-3xl font-bold text-blue-600 text-center"
        : "text-3xl font-bold text-sky-300 text-center" 
        }
        >SignUp</h1>
        <p className={isLight ? "mt-2 text-center text-gray-500":"mt-2 text-center text-white"}>
          Welcome to Kartly!!
        </p>
        ...
        <form className="mt-6 space-y-5" onSubmit={handleSignup}>
          {/* Email */}
          <div>
            <label className={isLight ? "block text-sm font-medium text-black":"block text-sm font-medium text-gray-300"}>Email</label>
            <input
              type="email"
          ...
          {/* Name */}
          <div>
            <label className={isLight ? "block text-sm font-medium text-black":"block text-sm font-medium text-gray-300"}>Name</label>
            <input
              type="text"
          ...
          {/* Password */}
          <div>
            <label className={isLight ? "block text-sm font-medium text-black":"block text-sm font-medium text-gray-300"}>Password</label>
            <div className="relative mt-1">
              ...
              />
              <span
                className={isLight ? "absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
